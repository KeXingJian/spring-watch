...
> **v2.1 修订**:原 C1"一定要做持久化"取消(经方案 A 评估代价过大,改走方案 C 纯内存),C2"允许丢失少量数据"也随 C1 取消而删除(无 WAL 就不存在 fsync 概念)。剩余 5 条约束仍然**不可违反**。

### 约束 C1:**不要求持久化(纯内存)**

- **解释**:**放弃** WAL / mmap / 任何磁盘持久化机制,数据仅存活在 JVM 堆里
- **数据丢失窗口**:**进程崩溃 = 当前所有 in-flight 数据全部丢失**
- **接受条件**:目标应用是**可重拉**的(指标 / 日志 / 心跳都有原始源,丢了就让目标应用重发)
- **数据完整性承诺**:只承诺**单进程运行期间**数据不丢(Producer offer 成功 + Consumer 未 drain = 数据还在 buffer 里)
- **错误反例**:不是 HertzBeat(无界 LinkedBlockingQueue)——本组件**严格有界** + **背压** + **多 partition**
