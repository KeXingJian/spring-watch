...
| **消息** | Kafka 3 / 3 / 1 partition(v1.5 收敛,白皮书 §0.5)+ `*.DLQ` 1 / 1 / 1 | Kafka 集群 + consumer 数 ≤ partition 数 |
...
- **Partition 对齐 `concurrency=3`**:`monitor-metrics` 3 partition / `monitor-logs` 3 partition / `monitor-heartbeat` 1 partition(v1.5 实测后从 12/6/3 收敛,详见白皮书 §0.5)
