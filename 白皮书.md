...
> **v1 现状(过渡方案)**:以下 6 步是当前 OTel Agent + AOP 切面方案的接入流程。
> **v2 目标(终态)**:自研 `spring-watch-agent.jar` 上线后,接入流程将简化为"1 个 -javaagent 参数",详见 §10 演进路线。
> 
> **接入成本硬上限(随版本演进):**
> | 版本 | JVM 参数 | 文件复制 | Maven 依赖 | 注解改动 | 业务改动 |
> |---|---|---|---|---|---|
> | **v1(当前)** | 1 | 2 | 1 | 1 行 | 1 个 `@SpringWatch` |
> | **v2(目标)** | 1 | 0 | 0 | 0 | 1 个 `@SpringWatch`(Agent 自带,无需 import) |
> 
> **当前 v1 文件复制清单(2 个)**:
> - `SpringWatch.java`(注解定义)
> - `SpringWatchAspect.java`(AOP 切面,被 OTel Agent 拦截的方法路径上织入)
> 
> **v1 复制 2 个文件的原因**:
> - OTel Java Agent **不会**自动读 `@SpringWatch` 自定义注解
> - 所以必须在客户项目内挂一个 AOP 切面拦截 `@SpringWatch` 方法
> - 这是 OTel + 自定义注解的"胶水代码",**自研 Agent 起来后这 2 个文件就消失**
...
### Q0(2026-06-16 新增):为什么当前要让客户复制 2 个文件到自己的项目?v2 会简化吗?

**答**:**v1 过渡方案**——目前 spring-watch 沿用 OTel Java Agent,但 OTel 不会自动读 `@SpringWatch` 自定义注解,所以**让客户复制 2 个文件(`SpringWatch` 注解 + AOP 切面)到自己的 `com.springwatch` 包**,通过 Spring AOP 织入监控逻辑。
...
- Agent 启动时扫描所有 class,识别 `@SpringWatch` 注解并字节码织入
- 客户**不再需要复制任何文件,不再需要 Spring AOP 依赖**
- 接入流程 = 1 步:`java -javaagent:spring-watch-agent.jar -jar my-app.jar`
- 方法调用栈完全在客户代码内,spring-watch 字节码不可见
...
### Q0.5(2026-06-16 新增):为什么当前让目标应用暴露 `GET /api/agent/logs?since=` 来拉日志?v2 会改吗?

**答**:**v1 过渡方案**——日志走"**HTTP GET 拉**"满足约束 1(拉取模型),但当前用 OTel Agent 时:
- OTel 默认不内置 log buffer + HTTP exporter
...
**v1 的代价**:
- 客户要引入 `spring-watch-log-pull-extension.jar`(Otel Agent 之外的额外扩展)
...
- 接入流程 = 1 个 `-javaagent`,**不再需要 `spring-watch-log-pull-extension.jar`**
...
| 版本 | 状态 | 核心特征 |
|---|---|---|
| **v1(当前)** | 过渡方案 | 沿用 OTel Java Agent + AOP 切面 + 日志拉取接口 |
| **v2(目标)** | 终态方案 | 自研 `spring-watch-agent.jar`,一步接入 |

**重要原则**:v1 → v2 演进过程中,**白皮书 5 大硬约束不变**,变化的是"用什么 Agent"和"怎么织入"。

### 10.2 v1(当前)与 v2(目标)对比

| 维度 | v1 现状(过渡) | v2 目标(终态) |
|---|---|---|
| **方法级指标织入** | OTel Agent + 客户项目内 AOP 切面(`SpringWatchAspect`) | 自研 Agent 字节码直接读 `@SpringWatch` 织入 |
| **接入文件复制** | 2 个文件(`SpringWatch` + `SpringWatchAspect`) | 0 |
| **接入 Maven 依赖** | `spring-boot-starter-aop` | 0 |
| **接入 `-javaagent` 参数** | 1 个 OTel Agent | 1 个自研 Agent |
| **接入 `@SpringBootApplication` 改动** | 1 行(scanBasePackages) | 0 |
| **`@SpringWatch` 注解** | 客户项目内复制 | 自研 Agent 自带,无需 import |
| **指标 /metrics 端点** | OTel Agent 暴露 | 自研 Agent 暴露(格式兼容 Prometheus) |
| **日志端点** | `GET /api/agent/logs?since=` 由 `spring-watch-log-pull-extension.jar` 暴露 | 自研 Agent 暴露(URL/格式 Agent 自定,仍走 GET) |
| **日志 buffer** | 内存 ringbuffer(`InMemoryLogBufferAppender`) | Agent 自带,可走磁盘 mmap 兜底 |
| **方法调用栈** | 进入 spring-watch 的 AOP 类 | 完全在客户代码内 |
...
- 1 个业务方法加 `@SpringWatch("xxx")`(注解由自研 Agent 自带,**无需 import,无需 copy**)

### 10.4 v1 → v2 迁移清单

| 迁移项 | v1 状态 | v2 处理 |
|---|---|---|
| `SpringWatchAspect.java` | 客户项目内 | **删除**,改由自研 Agent 字节码织入 |
| `SpringWatch.java` 注解定义 | 客户项目内 | **删除**,改由自研 Agent 自带 |
...
| OTel Java Agent(`opentelemetry-javaagent.jar`) | 客户启动时挂载 | **完全卸载** |
| `spring-boot-starter-aop` 依赖 | 客户 pom | **完全卸载** |
| 客户 `-javaagent:` 参数 | `-javaagent:opentelemetry-javaagent.jar ...` | 替换为 `-javaagent:spring-watch-agent.jar ...` |
