...
| 价值 | 落地                                                                                              |
|---|-------------------------------------------------------------------------------------------------|
| **轻量(Lightweight)** | 2C2G 单机可跑;JVM 堆稳态 ≤ 200 MB(13 目标 ~120 MB,500 目标 ~200 MB,数据来源 `docs/memory-architecture.md` 第 5 节) |
...
| **高可用(Resilient,非集群)** | 进程级自愈 + 进程内 InflightQueue 解耦 + 关键数据全落盘,**进程崩溃 → 30s 内自愈,落盘数据不丢**                                           |
...
| **内存** | 2 GB | 4 GB | 包含 spring-watch 进程(堆 ≤ 512 MB)+ PG + InfluxDB 共存(v1.6 已移除 Redis;采集→消费缓冲为进程内 InflightQueue,无外置队列) |
...
> 100 目标以下不需要任何 JVM / InfluxDB 调参,默认配置即用。
...
| **进程内队列 InflightQueue** | 纯内存 topic/partition 队列(`spring-watch.inflight.enabled: true`),替代采集 → 消费之间的内存 BlockingQueue,自带 P2C 路由 / 背压降级 / 进程内 DLQ / 自监控指标 | ❌ 进程内,不跨进程,故意不做外置 |
| **PostgreSQL 16** | 单节点 | ❌ 不部署主从 |
| **本地缓存 Caffeine** | 进程内 W-TinyLFU(替代 v1.5 之前的 Redis 7) | ❌ 故意不外置(白皮书 0.6.3) |
| **InfluxDB 2.7** | 单节点(`mem_limit: 1g` 兜底) | ❌ |

**InflightQueue 在本项目中的定位(关键澄清)**:

> InflightQueue **不是**为分布式扩容引入的,**不是**为多进程 / 多消费者并行引入的,**不是**为跨机房复制引入的。
>
> 它在 spring-watch 中的**唯一作用**:**替代"采集线程 → 消费线程"之间的内存 BlockingQueue**。
> 把"解析后的指标 / 日志 / 心跳"从采集层异步搬运到消费层,提供 topic 隔离、背压降级与进程内 DLQ。
> 纯内存、无 WAL、单进程,**不需要**外置中间件,也不预留任何"未来外置化"接口。

| 维度 | 原生 BlockingQueue(无队列层) | InflightQueue(本项目) |
|---|---|---|
| **topic / partition 隔离** | ❌ 单队列,高频写挤占低频写 | ✅ metrics 3 / logs 3 / heartbeat 1,互不阻塞 |
| **路由** | 无 | ✅ P2C(power-of-two-choices)按 partition pending 负载路由 |
| **背压** | ❌ 队列满阻塞采集线程 | ✅ 拒绝 + `BackpressureHandler` 降级兜底,采集线程不阻塞 |
| **失败兜底** | 无 | ✅ 进程内 DLQ topic + 重投 |
| **可观测** | 无 | ✅ `InflightMetrics`:sent / rejected / pending per partition |
| **运维成本** | 零 | 零(进程内,无容器) |
| **本项目选型** | ❌ 不可接受(无隔离、无背压、无兜底) | ✅ 选定 |

**为什么不外置集群?** 三条理由:

1. **单机定位**:队列只是"单机内的异步缓冲",数据最终落 PG / InfluxDB;进程崩溃时未消费数据由采集层按落库游标 / 周期重拉兜底,**不是**分布式协调
2. **降低运维成本**:中小企业和个人开发者不应该为监控平台单独雇一个 DBA / SRE;`docker compose up` 5 分钟拉起,`docker compose down` 5 分钟下线(外置组件仅 PG + InfluxDB)
...
**目标**:把指标和日志消费到 InfluxDB 写入的吞吐提上去。当前 inflight consumer 并发(每 topic 3)不是真瓶颈,**真瓶颈是 `WriteApi` 单例被 3 个 topic 抢 buffer**。
...
| **Inflight consumer (per topic)** | **3** | — | `spring-watch.inflight.consumer.concurrency: 3` |
| poll-max-batch | **200** | 100 msg/s/thread | 单批 200 条 |
...
| 阶段 | 改动 | 收益 | 风险 |
|---|---|---|---|
| **① 调 batch/flush 参数** | `poll-max-batch: 200→500`(inflight consumer)/ `write.batch-size: 1000→5000`(metrics 桶) / `write.flush-interval-ms: 2000→1000` / `write.buffer-limit: 20000→100000` | 单次 HTTP POST 体积大 5x,InfluxDB TSM 写入 batch 化更高效;buffer 容纳更多 burst | buffer 100K 条 × 200B = 20MB 堆占用,稳态 100-150MB 仍安全 |
| **② WriteApi 分桶** | 拆 4 个独立 WriteApi Bean:`metricsWriteApi`(batch=5000) / `logsWriteApi`(batch=3000) / `selfMetricsWriteApi`(默认) / `infraWriteApi`(默认,5 个基础设施 monitor 共享) | 3 个 topic 不再抢同一 buffer,metrics flush 不阻塞 logs flush;每个桶可独立调参 | 略增 InfluxDB HTTP 连接数(3→9,可忽略) |
...
| `infra_metrics` | `infraWriteApi` | 1000 | 2000ms | 20000 | 5 个基础设施 monitor + InfrastructureMetricsCollector |
...
**反模式(防误改)**:
- ❌ **禁止**:把 inflight consumer 并发调高(3→6)而不同时做 ① — buffer 一下就满,反而更慢
...
- ✅ **允许**:跑 1~2 周观察 `InflightMetrics`(sent / rejected / pending)与 buffer 占用,再决定是否调 batch / flush
...
| **采集→消费解耦** | 进程内 InflightQueue(topic/partition + P2C 路由 + 背压降级 + 进程内 DLQ),采集层与消费层异步解耦;队列纯内存、无 WAL,进程重启时未消费数据清空,由采集层按落库游标(`last_log_pull_time`)/ 周期重拉兜底 | `inflight/*` / `AppPullTask` |
| **数据持久化** | PostgreSQL 元数据(含日志拉取游标 `last_log_pull_time`)+ InfluxDB TSM 文件 **全部落盘**,重启不丢;**InflightQueue 与热路径状态(Caffeine)纯内存,重启即清空**(可接受,见 0.6.3) | `docker-compose.yml` volume 挂载 `.data/*` |
| **背压保护** | `HostThrottler` Per-Host 限流 + `PullRetryQueue` 失败重投 + `BackpressureHandler`(队列满拒绝 → 降级兜底),**OOM 不会发生** | `collector/schedule/*` / `inflight/*` |
...
- ❌ **禁止**:在 `@PostConstruct` 里调 `WriteApi.writePoints()` 之类可能依赖外部服务的操作
...
> **M-RedisOut**:v1.5 之前,`LogDedupService` / `AlertStateStore` / `LogAnomalyDetector` 三处热路径状态走 Redis 7。50 目标压测下,`monitor-logs` 队列 10 分钟堆积 50 万条、持续增长,根因是日志热路径单条 2-4 次 RTT(每条 ~3ms,3 线程上限 ~1k/s,而实际流量 50k/s)。**v1.6 全面迁出,改由 Caffeine(W-TinyLFU)本地承接**。详见 `docs/日志下游Caffeine优化报告.md`。
...
| **外置组件数** | 4 容器(PG / Redis / InfluxDB / Kafka) | **2 容器**(删 Redis;采集→消费缓冲改进程内 InflightQueue,删 Kafka) |
...
- ❌ **禁止**:在 `docker-compose.yml` 加回 `redis` / `kafka` 服务——**2 容器已收敛,加回是退步**
...
**为什么不用 PG `SELECT FOR UPDATE` 代替 Redis?** PG 行锁会拖死 `monitor-logs` consumer 吞吐,而且 PG 已经承受消费层的持续写入,QPS 上限不能打。**Caffeine 本地化是唯一解**。
...
- ❌ **禁止**:把 InflightQueue 当作"分布式消息系统"设计跨进程 topic、replica 选举、跨机房——本项目队列 = 单进程内存缓冲
...
- ✅ **允许**:InflightQueue 保持进程内,不为"分布式 / 外置化"预留 topic、partition 接口,只为解耦与背压,不调优吞吐
...
- ❌ **禁止**:把 inflight consumer 并发调到 > 6 而不先做 WriteApi 分桶(50 目标下 1 个 topic 6 个 consumer 足够,再高闲置)
...
- ✅ **允许**:跑 1~2 周 `InflightMetrics`(per-partition pending / sent / rejected)数据后,根据 pending 与消费速率调 batch / flush
- ✅ **允许**:把 `buffer-limit` 调到 100000(20MB 堆,稳态 100-150MB 仍安全)
- ✅ **允许**:新增 topic(比如未来的 `monitor-events`)时单独加一个 WriteApi Bean,不要复用现有的 4 个


#### 0.8.3 InflightQueue partition / 并发收敛反模式(2026-06-29 / v1.5 新增;队列化后沿用)
...
- ❌ **禁止**:把 consumer 并发调到 > 3 而不先加 partition(consumer ≤ partition,否则空转;见 `docs/memory-architecture.md` 6 第 2 条)
- ❌ **禁止**:为调 partition 数引入外置队列 / 集群化方案(partition 只是进程内配置,改 `spring-watch.inflight.partitions` 即可,不需要任何分布式工具)
- ❌ **禁止**:把 `InflightMetrics` 的 partition 观测关闭(关闭就失去 100 → 1000 目标扩容的判断依据,见 0.5 调研方法)
- ✅ **允许**:运行 1~2 周 `InflightMetrics` partition 数据后,如果 100 目标场景下 3/3/1/1 仍过冗余,可进一步收敛到 2/2/1/1
...
- ❌ **禁止**:在 `docker-compose.yml` 加回 `redis` / `kafka` 服务 — **2 容器已收敛**(PG + InfluxDB),加回是部署形态退步
- ❌ **禁止**:在 `pom.xml` 加回 `spring-boot-starter-data-redis` 依赖 — 同上
- ❌ **禁止**:把 `AlertStateStore` 改成 Redis 共享存储"支持多实例" — **直接违反 0.5 单机定位**,即使有 Redis 也不能支持多实例
- ❌ **禁止**:用 PG `SELECT FOR UPDATE` 代替 Caffeine 承接告警状态 — PG 已经承受消费层的持续写入,QPS 上限不能打
...
| **部署形态** | **单机全栈单实例**(详见 0.5),采集 → 消费由进程内 InflightQueue 解耦(纯内存 topic/partition);**热路径状态由 Caffeine 承接(详见 0.6.3)** |
...
graph LR
    ...
    D -->|MetricEvent| E[InflightQueue<br/>进程内 topic/partition]
...
| D → E | InflightQueue | 进程内队列解耦(采集 → 消费异步) |
| E → F | 消费者 | 必须消费队列再写 InfluxDB |
...
| **平台使用者(中小企业团队 / 个人开发者)** | 加注解暴露监控点、在 spring-watch Web 界面配置要监控的应用列表 | 写 spring-watch 拉取代码、运维 PG / InfluxDB(`docker compose up` 即可) |
...
- 写入进程内 InflightQueue(topic/partition + 背压降级 + 进程内 DLQ)
- 消费并写入 InfluxDB
- 提供查询 API(可选前端)

### ❌ spring-watch 不做什么

- **不接收**目标的任何 push(OTLP / WebSocket / gRPC stream)
...
### Q9(2026-06-29 / v1.3 新增,队列化后更新):采集 → 消费之间为什么用进程内 InflightQueue?为什么不用外置消息队列?

**答**:**InflightQueue 在 spring-watch 中的唯一作用是替代"采集 → 消费"之间的内存 BlockingQueue**,把"解析后的指标 / 日志 / 心跳"异步搬运到消费层。它是**进程内纯内存队列**(topic/partition 结构 + P2C 路由 + 背压降级 + 进程内 DLQ + 自监控指标),**不是**分布式消息系统,**不是**事件总线,**不是**为扩容设计的。

#### 为什么不用朴素的内存 BlockingQueue?

| 维度 | 朴素 BlockingQueue | InflightQueue(本项目) |
|---|---|---|
| **topic / partition 隔离** | ❌ 单队列,高频写挤占低频写 | ✅ metrics 3 / logs 3 / heartbeat 1 独立缓冲,互不阻塞 |
| **路由** | 无 | ✅ P2C(power-of-two-choices)按 partition pending 负载路由 |
| **背压** | ❌ 队列满阻塞采集线程 | ✅ 批量 offer all-or-nothing,满了走 `BackpressureHandler` 降级,采集线程不阻塞 |
| **失败兜底** | 无 | ✅ 进程内 DLQ topic + 定时重投 |
| **可观测** | 无 | ✅ `InflightMetrics`:`sent` / `rejected` / `pending`(per topic/partition) |
| **进程崩溃** | 未消费数据全丢 | 同左(纯内存无 WAL);由采集层按落库游标(`last_log_pull_time`)/ 周期重拉兜底,落盘数据不丢 |

#### InflightQueue 在数据流里的位置

```
[采集层] ── InflightProducerBridge ──> [InflightQueue(进程内 topic/partition)]
                                          │
                                          └─→ [InflightConsumer → BatchMetricConsumer / BatchLogConsumer / ...]
```

- **采集层**(collector/) 解析完 Prometheus 文本 / 日志 JSON 后,**异步**写入队列,**不**等待消费
- **消费层**(consumer/) 按 topic 并发(虚拟线程)批量拉取,写入 InfluxDB / PG
- **中间环节**用 InflightQueue 而不是朴素队列的核心收益:**topic 隔离 + 背压降级 + 失败重投 + 指标可观测**,且零外置组件

#### 关键澄清(防止后续误改)

- ❌ **禁止**:把 InflightQueue 当作"分布式消息系统"用,新增跨进程 topic、跨服务事件、跨服务协调
- ❌ **禁止**:把 InflightQueue 当作"分布式协调"用(leader 选举 / 分布式锁 / 集群元数据)
- ❌ **禁止**:为"未来集群化"引入外置消息队列(Redis Streams / RabbitMQ / 重新引入 Kafka)——**永远不会做集群**
- ❌ **禁止**:把 InflightQueue 当作"事件溯源 / CQRS / Saga 编排"的基础设施
- ❌ **禁止**:把 topic 数扩到 3 个以上(本项目只有 `monitor-metrics` / `monitor-logs` / `monitor-heartbeat` 3 个,加 DLQ 副本)
- ✅ **允许**:队列满时由 `BackpressureHandler` 降级兜底(拒绝 + 统计 + 采集侧重试,已实现,见 `inflight/BackpressureHandler.java`);长时间积压(> 30 分钟)由自监控告警介入
- ✅ **允许**:在自监控 / 告警规则里监控 InflightQueue 自身的 pending / rejected 指标(见 `InflightMetrics`)

#### 为什么不直接用 PostgreSQL / Redis 当队列?

| 方案 | 优 | 劣 |
|---|---|---|
| **InflightQueue**(选定) | 零外置组件、吞吐高、topic 隔离 + 背压 + 指标齐备 | 纯内存,进程重启未消费数据清空(采集侧重拉兜底) |
| **PG `NOTIFY` / LISTEN** | 零额外组件 | 消息不持久化(NOTIFY 队列在内存)、无消费 offset、文档少 |
| **Redis Streams / List** | 零额外组件(历史对比,**Redis 已移除**) | 持久化弱、ack 语义差,还要多维护一个外置组件 |
| **外置 Kafka / RabbitMQ** | 持久化语义强 | 多一个容器与学习成本,单机场景是过度设计 |

最终选定 **进程内 InflightQueue**:零外置组件、单机吞吐足够、topic 隔离与背压降级齐全,完全贴合单机定位。移除 Redis 与 Kafka 后,**外置组件仅剩 PG + InfluxDB 2 个**,部署形态收敛到 2 容器。
...
| **外部服务连不上** | `InfluxDBClient` 创建时连不上对应服务,卡在 `await()` | 启动前先 `docker compose up -d`(InflightQueue 为进程内队列,无外部依赖) |
...
### Q13(2026-06-30 / v1.6 新增):为什么日志下游突然变快了?是改了队列哪个参数?

**答**:**不是改队列,而是把日志 / 告警 / 异常分析的热路径状态从 Redis 迁到 Caffeine**(M-RedisOut 落地)。

#### 压测对比(50 目标,15s 一次拉取)

| 指标 | v1.5 之前(Redis 7) | v1.6(Caffeine) |
|---|---|---|
| `monitor-logs` 队列堆积 | **10 分钟堆积 50 万条,持续增长** | pending 动态 = 0 |
...
| 外置组件数 | 4 容器 | **2 容器**(删 Redis,删 Kafka) |
| 内存上限 | Redis 受 `maxmemory` 配置 | **80MB 硬封顶**(`maximumSize` W-TinyLFU) |
...
- ❌ **禁止**:在 `docker-compose.yml` 加回 `redis` / `kafka` 服务
...
| **appid** | 监控目标在 spring-watch 平台注册时系统生成的 **53 bit 雪花 ID**(Long),唯一且不可变,贯穿队列路由 key、InfluxDB tag、OTel `OTEL_SERVICE_NAME`、API 查询参数。是定位指定监控目标的**关键条件** |
