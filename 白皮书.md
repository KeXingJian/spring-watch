...
| 资源 | 单机部署基线                      | 上限建议   | 备注 |
|---|-----------------------------|--------|---|
| **CPU** | 2 核                         | 4 核    | 13 ~ 100 目标时单核 < 30% 占用 |
...
- **v1.2+**:`@SpringWatch` + `code-function-metrics`(OTel 原生)
...
### Q6:客户想用 @SpringWatch 怎么办?

**答**(2026-06-18 更新):`@SpringWatch` + `code-function-metrics=true` 是 **v1.2+ 主推方案**,spring-watch **可以拉**对应 metric(走 `/metrics` 端点的 `code.*` 命名空间)。
