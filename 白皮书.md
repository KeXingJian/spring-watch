...
**当前 Kafka partition 配置 vs 单机定位的张力(v1.3 待调研收敛 → v1.5 实测后收敛)**:

白皮书 0.5 的"单 partition 足够"是从吞吐测算推出来的理论值,但当前 `application.yml` 实际配置是:

| topic | v1.4 配置 | v1.5 收敛后 | 理论最小(对齐 `concurrency:3`) | 备注 |
|---|---|---|---|---|
| `monitor-metrics` | 12 | **3** | 3 | v1.4 是按"留 4 倍冗余"拍脑袋定的,v1.5 实测后收敛 |
| `monitor-logs` | 6 | **3** | 3 | 同上 |
| `monitor-heartbeat` | 3 | **1** | 1 | 心跳是低频,1 足够 |
| `.DLQ`(metrics / logs) | 3 | **1** | 1 | 死信极低频 |
| `.DLQ`(heartbeat) | 1 | 1 | 1 | 不变 |

**v1.4 时为什么不直接调到理论值?** 因为理论值是"单盘顺序写 + 单 consumer"的极限,实际场景下还要考虑:
...
**所以 v1.4 的 12/6/3/3 是"保守但留扩展空间"的初始值**,v1.3 上线 `KafkaPartitionUtilizationGauge` 调研 1~2 周,根据实际 produced_rate / consumed_rate 数据决定是否收敛。

> 调研项见 `docs/observability-plan.md` C.6 + `KafkaPartitionUtilizationGauge.java`。

**v1.5 调研结论(2026-06-29 实测后收敛)**:

跑了约 24h `KafkaPartitionUtilizationGauge`(`infra_metrics` 桶,measurement=`kafka.partition.utilization`,tag=`{component=kafka, metric=kafka.partition.utilization, topic, partition}`),24h 均值如下:

| topic | partition 数 | 总 produced_rate (msg/s) | min-max (msg/s) | 24h max (msg/s) | 结论 |
|---|---|---|---|---|---|
| `monitor-metrics` | 12 | **48.78** | 1.32 ~ 9.24 | 单 partition peak 187.6(p5) | 全部 partition < 10 msg/s,即使收敛到 3 个 partition,单 partition 平均 16 msg/s、peak 308 msg/s,单 consumer 完全吃得下 |
| `monitor-logs` | 6 | **71.42** | 5.76 ~ 19.44 | 单 partition peak 435.8(p5) | 全部 < 20 msg/s,收敛到 3 个 partition,单 partition 平均 24 msg/s、peak 449 msg/s,单 consumer 完全吃得下 |
| `monitor-heartbeat` | 3 | **0.13** | 0.04 ~ 0.05 | 单 partition peak ~0.83 | 3 个 partition 都 < 0.1 msg/s,**100% 冗余**,直接 1 |

**对照白皮书 §0.5 决策表**:

| 决策表条件 | 实测匹配 | 决策 |
|---|---|---|
| 所有 partition `consumed_rate` 均匀且都 < 10 batch/s | ✅ `monitor-metrics` 全部 < 10 msg/s,`monitor-logs` 全部 < 20 msg/s | 满足 |
| 1~2 个 partition 持续 > 100 msg/s,其他 < 10 msg/s | ❌ 24h 均值所有 partition 都 < 20 msg/s,只有日志 p5 在 peak 时到 435 | 不满足(peak 不是 "持续") |
| 所有 partition 都 > 50 msg/s | ❌ 24h 均值所有 partition < 20 msg/s | 不满足 |
| 单 partition 写入打满单盘 IO | ❌ 写入 IO 远低于单盘上限 | 不满足 |
| `produced_rate` 持续 < `consumed_rate` | ❌ produced ≈ consumed,健康 | 不满足 |

**结论**:**满足第 1 条 + 不满足第 2~4 条 = "已均匀且低利用率"**,按白皮书 §0.5 决策表收敛到 3/3/1/1。

**为什么 3 个 partition 而不是 1 个?** 保留 3 个 partition 的两个理由:
1. **hot-key isolation 仍要保留**:`KafkaProducerBridge` 用 `appid` 当 key,100 目标下 ~100 个 appid 哈希到 3 个 partition,平均每 partition ~33 appid;单 app 突发流量被该 appid 所在的 partition 隔离,不会拖慢其他 appid
2. **对齐 `concurrency:3`**:3 个 partition 正好 1:1 分给 3 个 consumer,无空转、无浪费(consumer ≤ partition,见 `docs/memory-architecture.md` §6 第 2 条)

**`monitor-heartbeat` / `*.DLQ` 收敛到 1** 的额外理由:
- heartbeat 单目标 15s 一次心跳(白皮书 §1 约束 1),100 目标下 6.7 msg/s 总量,1 partition 足够
- DLQ 死信极低频(99% 时间 0 msg/s,异常时偶发),1 partition 足够

**反模式(防误改)**:
- ❌ **禁止**:把 partition 数再往上加(本项目 100 目标下 3/3/1/1 已达上限,1000 目标请评估迁移到 Prometheus,见 §0.8)
- ❌ **禁止**:把 partition 数再往下减到 1(`monitor-metrics` / `monitor-logs` 各保留 3 个用于 hot-key isolation,100 目标下有意义)
- ❌ **禁止**:把 `concurrency` 调到 > 3 而不先加 partition(consumer ≤ partition,否则空转)
- ❌ **禁止**:用 `kafka-topics.sh --alter --partitions` 调高 partition 数后不验证 consumer 重新分配(可能造成 partition rebalance 期间 consumer lag 短暂冲高)
- ✅ **允许**:运行 1~2 周 `KafkaPartitionUtilizationGauge` 数据后,如果 100 目标场景下 3/3/1/1 仍过冗余,可在白皮书 v1.6 进一步收敛到 2/2/1/1
- ✅ **允许**:新增 topic(比如未来的 `monitor-events`)时按本节决策表单独评估,不要复用现有 partition 配置

**迁移说明(已有部署)**:
Kafka 协议禁止减 partition 数,无法通过 AdminClient/UI 减少。已有部署收敛到 3/3/1/1 的步骤:
```bash
# 1) 停 spring-watch
docker compose stop spring-watch
# 2) 删 Kafka 数据目录(本项目用 docker volume,数据可重建)
rm -rf .data/kafka_data
# 3) 改 application.yml(本 PR 已改)
# 4) 重启,KafkaTopicConfig.onApplicationReady() 兜底按新值创建
docker compose up -d spring-watch
```

> ⚠️ 此操作会丢 Kafka 内未消费的数据;开发环境可接受,生产环境请评估。生产收敛建议用 `kafka-topics.sh --alter` 先扩到 ≥ 3/3/1/1 中较大的值(已是 12/6/3),然后让 `KafkaTopicConfig` 兜底匹配,或者使用 Kafka MirrorMaker2 做迁移(超出本项目范围,不在 spring-watch 单机定位内,见 §0.5)。
...
**部署形态反模式**:
- ❌ **禁止**:给 `AlertStateStore`(进程内 Caffeine 状态机)加 Redis / 任意外置 KV 共享存储,试图支持多实例——**直接违反 0.5 单机定位**(v1.6 已全量迁到 Caffeine,见 §0.6.3)
...
- ❌ **禁止**:在 `docker-compose.yml` 加回 `redis` 服务——v1.6 已删,**3 容器已收敛**(PG + InfluxDB + Kafka)
...
- ❌ **禁止**:给 InfluxDB / PG 加 replica 配置,试图"为 HA 加副本"——单实例足够,出问题靠备份恢复(v1.6 已无 Redis)
...
**Caffeine 本地缓存反模式(2026-06-30 / v1.6 M-RedisOut 沉淀)**:
- ❌ **禁止**:把 `LogDedupService` / `AlertStateStore` / `LogAnomalyDetector` 任一改回 Redis / 任意外置 KV(已踩坑:50 目标压测 10 分钟堆积 50 万日志,根因是 Redis RTT ~3ms/条)
- ❌ **禁止**:在 `docker-compose.yml` 加回 `redis` 服务 — **3 容器已收敛**(PG + InfluxDB + Kafka),加回是部署形态退步
- ❌ **禁止**:在 `pom.xml` 加回 `spring-boot-starter-data-redis` 依赖 — 同上
- ❌ **禁止**:把 `AlertStateStore` 改成 Redis 共享存储"支持多实例" — **直接违反 0.5 单机定位**,即使有 Redis 也不能支持多实例
- ❌ **禁止**:用 PG `SELECT FOR UPDATE` 代替 Caffeine 承接告警状态 — PG 行锁会拖死 `monitor-logs` consumer 吞吐,PG 已经承受 5 个 Kafka topic 写入
- ❌ **禁止**:把 Caffeine cache 的 `maximumSize` 设成 `Long.MAX_VALUE` — 必须硬封顶,否则内存不可控
- ❌ **禁止**:`@PostConstruct` 初始化 Caffeine cache 并预设大量元素 — `@PostConstruct` 只做无副作用操作,大量预设推迟到 `@EventListener(ApplicationReadyEvent.class)`
- ❌ **禁止**:把 `expireAfterWrite` 调成 `Long.MAX_VALUE`(等于无限 TTL) — 必须有自然上限配合 `maximumSize` 双保险
- ❌ **禁止**:把 `evict_size` 告警阈值设为 0 — 该指标是"流量超配"信号,必须 `> 0` 触发
- ❌ **禁止**:把 `flush_fail` 告警阈值设为 0 — `log_dedup_count` 双写失败意味着统计计数即将丢失
- ✅ **允许**:为 4 个核心 cache(`dedupCache` / `stateCache` / `errorRateCache` / `knownPatternsCache`)分别配独立 `evict_size` 告警
- ✅ **允许**:增大 `max-entries` 应对突发流量,但需同步调整告警阈值
- ✅ **允许**:用 Caffeine `recordStats()` + Micrometer 桥接,导出 `cache_size` / `cache_max` / `evict_size` / `evict_expired` / `hit_count` / `miss_count` 6 个指标
- ✅ **允许**:进程重启后让告警状态重判 PENDING、dedup 重置窗口 — 这是 v1.6 主动选择的 Trade-off(可接受)
- ✅ **允许**:用 `asMap().putIfAbsent` 做无锁 SETNX,`asMap().compute()` 做 per-key 串行 CAS(等价 Redis Lua 脚本)
